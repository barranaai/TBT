 ...
 * Description: Services, testimonials, public smile transformations and page SEO for Teeth by Trev. Separate from enquiries and payments.
 * Version: 1.0.0
...
define( 'TBT_CONTENT_VERSION', '1.0.0' );
...
function tbt_content_init(): void {
	foreach ( array( 'tbt_service' => array( 'Services', 'Service', 'dashicons-heart' ), 'tbt_testimonial' => array( 'Testimonials', 'Testimonial', 'dashicons-format-quote' ), 'tbt_smile' => array( 'Smile Transformations', 'Transformation', 'dashicons-format-gallery' ) ) as $type => $labels ) {
...
add_action( 'save_post', static function ( $id ) {
	if ( in_array( get_post_type( $id ), array( 'tbt_service', 'tbt_testimonial', 'tbt_smile', 'page' ), true ) && ! wp_is_post_revision( $id ) ) { tbt_content_clear_cache(); }
}, 100 );

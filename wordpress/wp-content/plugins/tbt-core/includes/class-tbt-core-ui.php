...
final class TBT_Core_UI {
	...
	public static function assets(): void {
		...
		if ( is_page( 'contact' ) ) {
			...
			wp_localize_script( 'tbt-inquiry', 'TBTInquiryConfig', array( 'endpoint' => rest_url( 'tbt/v1/inquiry' ), 'consentVersion' => '2026-08-10-v3', 'teamHandle' => '@teethbytrev.team', 'teamUrl' => 'https://www.instagram.com/teethbytrev.team/' ) );
		}
		if ( is_page( 'reserve' ) ) {
	...
	private static function select( string $name, string $label, array $options, bool $required = true, string $intent = '' ): void {
		$required_attr = $required ? ' required data-required="true" aria-required="true"' : '';
		?>
	...
	private static function photo_upload( string $id, string $note ): void {
		?>
		<div class="tbt-photo-field" data-tbt-photo-field><p id="<?php echo esc_attr( $id ); ?>-label" class="tbt-label">Smile photos <span class="text-gold">*</span></p><label for="<?php echo esc_attr( $id ); ?>" class="tbt-photo-drop"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.4" aria-hidden="true" class="h-8 w-8 text-gold"><path d="M4 8.5h3l1.3-2h7.4l1.3 2h3v10H4z"></path><circle cx="12" cy="13" r="3"></circle></svg><span class="mt-3 text-sm text-ivory/70"><span class="text-gold">Tap to upload photos</span></span><span class="mt-1 text-xs text-ivory/55">Front view, side view, and any areas of concern</span><input id="<?php echo esc_attr( $id ); ?>" type="file" accept="image/*,.heic,.heif" multiple required aria-required="true" aria-labelledby="<?php echo esc_attr( $id ); ?>-label" class="sr-only" data-tbt-photos></label><ul class="mt-4 flex flex-wrap gap-2" data-tbt-photo-list></ul><p class="mt-3 text-xs text-ivory/55"><?php echo esc_html( $note ); ?></p></div>
		<?php
	}
			...
			<form data-tbt-wizard class="hidden" novalidate>
				...
				<div class="relative border border-ivory/10 bg-white/[0.02] p-6 sm:p-10 lg:p-12"><span class="pointer-events-none absolute left-0 top-0 h-12 w-px bg-gold/50"></span><span class="pointer-events-none absolute left-0 top-0 h-px w-12 bg-gold/50"></span><p class="text-[0.6rem] uppercase tracking-[0.34em] text-gold/70" data-tbt-step-meta></p><h2 class="mt-4 font-serif text-3xl font-light leading-[1.1] text-ivory sm:text-4xl" data-tbt-step-title></h2>
					...
					<section class="mt-10 space-y-6" data-step="contact">
						<div class="grid gap-6 sm:grid-cols-2"><div class="tbt-field"><label class="tbt-label" for="firstName">First name <span class="text-gold">*</span></label><input class="tbt-input" id="firstName" name="firstName" autocomplete="given-name" required aria-required="true" data-required="true"></div><div class="tbt-field"><label class="tbt-label" for="lastName">Last name <span class="text-gold">*</span></label><input class="tbt-input" id="lastName" name="lastName" autocomplete="family-name" required aria-required="true" data-required="true"></div></div>
						<div class="grid gap-6 sm:grid-cols-2"><div class="tbt-field"><label class="tbt-label" for="phone">Mobile number <span class="text-gold">*</span></label><input class="tbt-input" id="phone" name="phone" type="tel" autocomplete="tel" inputmode="tel" placeholder="(424) 000-0000" required aria-required="true" data-required="true"></div><div class="tbt-field"><label class="tbt-label" for="email">Email address <span class="text-gold">*</span></label><input class="tbt-input" id="email" name="email" type="email" autocomplete="email" inputmode="email" placeholder="you@example.com" required aria-required="true" data-required="true"></div></div>
						...
						<div class="tbt-field border-l border-gold/45 pl-5"><label class="tbt-label" for="socialHandle">Your Instagram username <span class="text-gold">*</span></label><input class="tbt-input" id="socialHandle" name="socialHandle" placeholder="@yourusername" autocomplete="off" required aria-required="true" data-required="true"><p class="mt-3 text-xs leading-relaxed text-ivory/55">This remains required for every enquiry and contact method so the concierge team can match your profile.</p></div>
					</section>
					...
					<section class="mt-10 hidden space-y-9" data-step="new-smile" data-intent-panel="new">
						...
						<div class="tbt-field"><label class="tbt-label" for="goals">Your goals and concerns <span class="text-gold">*</span></label><textarea class="tbt-input resize-none" id="goals" name="goals" rows="5" placeholder="What would you like to change or improve about your smile?" required aria-required="true" data-required="true"></textarea></div>
					...
					<section class="mt-10 hidden space-y-10" data-step="new-investment" data-intent-panel="new">
						...
						<fieldset class="tbt-choice-field" data-required="false" data-choice-field="videoConsult" data-multiple="false"><legend class="tbt-label">Interested in a private video consult with Dr. Trev?</legend><div class="flex flex-wrap gap-3"><button type="button" class="tbt-choice" data-choice-value="Yes" aria-pressed="false">Yes, I am interested</button><button type="button" class="tbt-choice" data-choice-value="No" aria-pressed="false">Not right now</button></div><p class="mt-3 text-xs leading-relaxed text-ivory/55">A $250 deposit reserves a private video consultation and is credited 100% toward your treatment.</p></fieldset>
					</section>
					...
					<section class="mt-10 hidden space-y-8" data-step="general-enquiry" data-intent-panel="general">
						...
						<div class="tbt-field"><label class="tbt-label" for="message">Message <span class="text-gold">*</span></label><textarea class="tbt-input resize-none" id="message" name="message" rows="6" required aria-required="true" data-required="true"></textarea></div>
					...
					<section class="mt-10 hidden space-y-8" data-step="permission">
						...
						<label class="tbt-consent"><input id="contactConsent" name="contactConsent" type="checkbox" class="mt-1 h-4 w-4 shrink-0 accent-gold" required aria-required="true" data-required="true"><span class="text-sm leading-relaxed text-ivory/80">I authorize the Teeth by Trev concierge team to contact me about this enquiry and my care using my selected contact method, and by email where needed to respond. If I selected Instagram, messages may come from the official account <span class="text-gold">@teethbytrev.team</span>. <span class="text-gold">*</span></span></label>
						<label class="flex cursor-pointer items-start gap-4 border border-ivory/12 p-5 transition-colors hover:border-ivory/25"><input id="smsConsent" name="smsConsent" type="checkbox" class="mt-1 h-4 w-4 shrink-0 accent-gold"><span class="text-sm leading-relaxed text-ivory/70">Optional: I agree to receive service text messages (SMS) from Teeth by Trev at the mobile number provided — appointment availability, booking and payment confirmations, rescheduling, and follow-ups. Message frequency varies. Message and data rates may apply. Consent is not a condition of purchase. Reply STOP, END, QUIT, CANCEL or UNSUBSCRIBE to opt out, or HELP for help. See our <a href="<?php echo esc_url( home_url( '/privacy/' ) ); ?>" class="text-gold underline-offset-4 hover:underline">Privacy Policy</a> and <a href="<?php echo esc_url( home_url( '/terms/' ) ); ?>" class="text-gold underline-offset-4 hover:underline">Terms &amp; Conditions</a>.</span></label>
						<label class="flex cursor-pointer items-start gap-4 border border-ivory/12 p-5 transition-colors hover:border-ivory/25"><input id="marketingConsent" name="marketingConsent" type="checkbox" class="mt-1 h-4 w-4 shrink-0 accent-gold"><span class="text-sm leading-relaxed text-ivory/70">Optional: I would like to receive occasional Teeth by Trev news and offers by email. I can unsubscribe at any time. This does not sign me up for text messages.</span></label>
